...
class UIDialog extends UIDiv {
	constructor(content, title) {
		...
		this.close = new UIButton('╳')
		this.close.addClass('Close')
		this.close.dom.addEventListener('click', e => this.dom.remove(), false)
		...
		dragElement(this.dom, this.header.dom)
	}
}
...
function dragElement(el, header = el) {
	...
	function dragMouseDown(e) {
		...
		positions[2] = ev.clientX
		positions[3] = ev.clientY

		bounds.top = el.offsetHeight / 2
		bounds.left = el.offsetWidth / 2
		bounds.right = window.innerWidth - el.offsetWidth / 2
		bounds.bottom = window.innerHeight - el.offsetHeight / 2
	...
	function elementDrag(e) {
		...
		positions[0] = positions[2] - ev.clientX
		positions[1] = positions[3] - ev.clientY
		positions[2] = ev.clientX
		positions[3] = ev.clientY

		el.style.top = clamp(
			el.offsetTop - positions[1],
			bounds.top,
			bounds.bottom
		) + 'px'

		el.style.left = clamp(
			el.offsetLeft - positions[0],
			bounds.left,
			bounds.right
		) + 'px'
	}
...
const openDialog = (content, title) => {
	const contentElement = typeof content == 'string' ? new UIText(content) : content
	const titleElement = typeof title == 'string' ? new UIText(title) : title

	document.body.appendChild(
		new UIDialog(contentElement, titleElement).dom
	)
}
...
export { UIElement, UIAnchor, UISpan, UIDiv, UIRow, UIPanel, UIText, UIInput, UITextArea, UISelect, UICheckbox, UIColor, UINumber, UIInteger, UIBreak, UIHorizontalRule, UIButton, UIProgress, UITabbedPanel, UIListbox, ListboxItem, UIDialog, openDialog, closeDialog };
